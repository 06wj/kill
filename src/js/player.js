...
var Player = window._Player = Class.create({
    ...
    Statics:{
        init:function(_game){
            game = _game;
            mediator.on("playerDied", function(e){
                if(!config.god){
                    e.detail.player.die();
                }
            });
    ...
    onUpdate:function(){
        ...
        if(this.x + this.halfWidth > game.right){
            this.x = game.right - this.halfWidth;
        }
        else if(this.x - this.halfWidth < game.left){
            this.x = game.left + this.halfWidth;
        }

        if(this.y + this.height > game.bottom){
            this.y = game.bottom - this.height;
        }
        else if(this.y < game.top){
            this.y = game.top;
        }
